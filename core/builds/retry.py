...
* ``ingest`` is SKIPPED for a retry (``core.builds.stages._retry_parent``): the
...
async def graph_entangles_failed_docs(
    conn: AsyncConnection,
    parent_build_id: uuid.UUID,
    failed_content_hashes: frozenset[str],
) -> bool:
    """Whether any parent ENTITY has a text mention from BOTH a failed doc AND a
    non-failed doc — a state RB1-retry-skip's selective clone can't faithfully reuse.

    Both entity AND relation ROWS are first-write-wins: ``extract_documents`` mints
    the row on the first chunk to hit its ``entity_key`` / ``relation_signature`` and
    NEVER updates its scalar fields (``canonical_name``/``type``/``disambiguator``/
    ``attributes`` for entities, ``confidence``/``attributes`` for relations —
    ``core/graph/documents.py``); a later chunk only adds a mention/evidence. So if a
    FAILED document first-wrote a row from a chunk that ran BEFORE its failure, the
    row carries that PARTIAL value (e.g. a mis-cased ``canonical_name``); cloning it
    (the row is "successful" because a non-failed doc also touches the key) copies
    that value, and the child's ``preload`` blocks the re-extraction from correcting
    it — diverging from a full rebuild. Rows record no creating-doc, so any ENTITY
    mentioned by both a failed and a non-failed document forces a full re-derive.

    Checking ENTITIES SUBSUMES the relation case (Codex #103 R3+R4): a chunk-evidence
    relation exists only because a chunk accepted BOTH its endpoints (documents.py's
    per-chunk ``accepted_keys`` requires a text mention for each), so an entangled
    relation's endpoints are themselves entangled entities and this predicate catches
    it. An entity touched ONLY by failed docs is already excluded from the clone (no
    success mention); one touched only by non-failed docs is genuinely reusable.
    Follow-up (a future slice): re-extract just the entangled successful docs.

    Considered residual: a row's scalar first-writer is inherently nondeterministic
    across runs anyway — ``extract_documents`` iterates documents with no ``ORDER
    BY`` — so exact scalar match to a rebuild was never guaranteed, and the
    ``merged==rebuild`` oracle asserts on identity/evidence sets, not confidence. This
    guard removes the SYSTEMATIC bias where a failed doc's partial write is retained;
    the residual nondeterminism sits inside that same pre-existing tolerated envelope.
    """
    m = tables.entity_mentions
    e = tables.entities
    failed = sorted(failed_content_hashes)
    fm = m.alias("fm")  # a text mention from a FAILED doc …
    sm = m.alias("sm")  # … and one from a non-failed doc, on the SAME entity
    # entity_mentions has no build_id — scope through the entity to the parent build.
    entangled = (
        sa.select(sa.literal(1))
        .select_from(
            fm.join(sm, fm.c.entity_id == sm.c.entity_id).join(e, e.c.id == fm.c.entity_id)
        )
        .where(
            e.c.build_id == parent_build_id,
            fm.c.source_kind == "text",
            sa.func.split_part(fm.c.source_ref, ":", 2).in_(failed),
            sm.c.source_kind == "text",
            sa.func.split_part(sm.c.source_ref, ":", 2).notin_(failed),
        )
        .limit(1)
    )
