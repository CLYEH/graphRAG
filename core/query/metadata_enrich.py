...
async def enrich_response_metadata(
    ...
    if not exposure.fields:
        return response
    chunk_ids = _chunk_ref_ids(response)
    if not chunk_ids:
        return response
    exposed_by_chunk = await _exposed_metadata_by_chunk(repo, chunk_ids, exposure)
    if not exposed_by_chunk:
        return response
    return dataclasses.replace(
        response,
        results=tuple(_enrich_result(result, exposed_by_chunk) for result in response.results),
    )


def _chunk_ref_ids(response: McpResponse) -> set[uuid.UUID]:
    ids: set[uuid.UUID] = set()
    for result in response.results:
        for ref in result.source_refs:
            if ref.source_type == "chunk" and (parsed := _as_uuid(ref.id)) is not None:
                ids.add(parsed)
    return ids
...
async def _exposed_metadata_by_chunk(
    repo: BuildScopedRepo, chunk_ids: set[uuid.UUID], exposure: MetadataExposure
) -> dict[uuid.UUID, dict[str, Any]]:
    """Resolve chunk → document (two build-scoped bulk reads) and project each
    document's envelope through the allowlist. Only chunks whose document has a
    non-empty exposed projection appear in the result."""
    ...
    exposed: dict[uuid.UUID, dict[str, Any]] = {}
    for chunk_id, document_id in document_by_chunk.items():
        envelope = envelope_by_document.get(document_id)
        if isinstance(envelope, Mapping):
            projected = exposure.project(envelope)
            if projected:
                exposed[chunk_id] = projected
    return exposed


def _enrich_result(
    result: RetrievalResult, exposed_by_chunk: dict[uuid.UUID, dict[str, Any]]
) -> RetrievalResult:
    refs = tuple(_enrich_ref(ref, exposed_by_chunk) for ref in result.source_refs)
    return dataclasses.replace(result, source_refs=refs)


def _enrich_ref(ref: SourceRef, exposed_by_chunk: dict[uuid.UUID, dict[str, Any]]) -> SourceRef:
    if ref.source_type != "chunk":
        return ref
    chunk_id = _as_uuid(ref.id)
    exposed = exposed_by_chunk.get(chunk_id) if chunk_id is not None else None
    if not exposed:
        return ref
