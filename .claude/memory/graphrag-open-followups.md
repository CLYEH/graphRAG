...
- **hybrid 的 `relation_hit_rate` 可能因 passage 樓地板位移(2026-07-27, QA2 #142 gate-2 r2)**:`_chunk_floor` 把 chunk 以外的**所有**型別(entity/relation/path/row)併成同一個 `top_k//2` 桶,而 `core/eval/scoring.py:91-96` 的 `relation_hit_rate` 只數 `relation`/`path` 結果——同一問題下 relation/path 能佔的頁面份額因此可能變小,hybrid 的 golden 分數隨之位移。**現況非破口**:倉內沒有任何已 commit 的 golden 使用 `must_include_relations`(已 commit 的兩套 `projects/{nmmst,museum}/eval/golden.yaml` 目前全為 `mode: semantic`、皆無 `must_include_relations`;OpenAPI 範例外無 `mode: hybrid` 案例),且 `poe check` 不受影響(`tests/test_eval_scoring.py` 直接建 response)。**診斷 eval 位移前先查本條**:若 hybrid golden 的 relation 命中率下降,先確認是不是樓地板的份額效應而非抽取/檢索退步。真正要動時的選項:relation/path 自成一桶,或 `relation_hit_rate` 改看 source_refs 而非 result_type。
