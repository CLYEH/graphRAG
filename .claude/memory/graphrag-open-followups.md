...
- **`data/` 未 gitignore 使 receipt tree ≠ commit tree(2026-07-26, QA5 #140 gate-2 發現)**:`data/` 是 untracked 但**未**進 .gitignore,而 receipt 與 gate 腳本用 `git add -A` 算 content-addressed tree,故執行中的 dev server 寫進 `data/uploads/...` 的檔案會被算進被蓋章的 tree(已實見 `data/uploads/museum/49ff80ba….txt`);但提交時用 `git add -A ':!data'`,兩者集合不同——review→push 之間任何一次上傳都會**靜默失效**全部三張 receipt(且失效原因與程式碼無關,極難診斷)。修法:`data/` 加進 .gitignore,使被證明的 tree 與被推送的 tree 對齊,並消掉一整類 spurious tree churn。屬 harness 衛生,非 QA5 範圍。
