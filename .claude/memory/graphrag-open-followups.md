...
**同條另記兩個 QA7 有意識接受的殘留(2026-07-27, #145 Codex+gate-2)**:(i) **recreate 微競態**——corpus 目錄以**名字**定址,而名字在該列刪除後即可重用;多個單-worker 實例共用 corpus root 是**文件化的擴充形態**(`cli/main.py:95`),故 commit 後的 SoR 覆查與 rmtree 之間仍有一個微秒級窗口。**這是活路徑,不是遺留狀態**(與上半段不同,S10 的「入帳即宣稱現在沒壞」在此不適用);已把失敗側從「刪掉活專案的語料」翻成「洩漏」,但不為零。(ii) **rmtree 於 commit 後失敗**→目錄孤兒 + 一個「其實成功了」的 500,重試會 404 而永不再清。**兩者的正解是同一個**:corpus 目錄改用**不可重用的 project id** 命名(generation-unique),而非可重用的名字。**為何自成一任務**:該目錄名不是內部細節——`api/routers/uploads.py:218-226` 把 `corpus_dir.as_uri()` 寫進 `sources.uri`(DR-010),ingest 再讀回,故這是一次**跨 SoR 的資料遷移**(含既存語料),不是改個名字。**注意**:P2 不可在 layout 之前單獨修——「durable cleanup queue」這個顯而易見的解法會**放大** (i):把 recreate 窗口從微秒拉長到佇列深度。兩者必須一起解。
