...
- **Codex P2/P3 意見判讀政策(使用者 2026-07-03 定案;H2 任務會寫入 LOOP.md step 7,落地後以該文為準)**:P2/P3 級意見先判讀必要性——**必要**=違反 DESIGN 凍結保證/契約或文件內部矛盾/會誤導未來實作/真 bug;**非必要**=超出 DESIGN 文字的假設性強化、風格偏好、無互通性依據的凍結要求 → 回覆**引用 DESIGN §/DR 依據**後 resolve 不修;寫不出引用=該修。判讀模糊時 stop and ask。+1 門檻不變。
