...
**How to apply:** push → 立刻(或 ~60s 後)`gh pr checks <pr>` → 有紅燈**先修**(governance / lint / test / cov)→ 綠了再啟 Codex watcher。特別記住 **governance job** 規則:`task/<id>` 分支必須恰好勾掉 TASKS.md 的 `<id>` 項且不勾別項——**切片任務時分支名要對到子項**(C3a 的 PR 走 `task/C3a`,不是 `task/C3`),否則 `scripts/governance-check.sh` 會擋。相關 [[graphrag-lesson-classes]]。
...
**等 Codex 一律用 `scripts/watch-codex.sh`,禁止土製輪詢(2026-07-13,#72 教訓——owner 抓到「反智行為」)**:Codex 的 changes-wanted 以 **review** 管道送達;ad-hoc loop 只看 reactions+comments-after-timestamp 就會把「已有兩條 P2」誤判成「無回應」→ 錯誤地 poke。這正是 watcher 註解裡寫明的 PR#5 failure mode——工具就是為這個翻車模式建的,repo 有現成工具先用,別重寫弱化版。watcher exit code:0=+1、10=新回應要 triage、20=timeout、30=quota。

**PR 寫入前 fresh-state check(owner 2026-07-13 協議)**:對 PR 的**每個寫入**(comment/reply/resolve/poke/merge)都要在**同一個腳本**裡先重查當下狀態(unresolved threads、bot 最新回應時間 vs head commit 時間、reactions),前提仍成立才發送,否則中止並回報——**用程式判斷,不要 LLM 讀**(呼應 global Rule 5)。已知的兩個具體判準:(a) poke 前先驗「Codex 最新回應時間 < head commit 時間」(已回應就 triage,不 poke);(b) merge hook 要求 +1 **晚於** head commit——push 之後舊 +1 失效,需 fresh review;等待用 watch-codex.sh,不要自寫 sleep loop。
