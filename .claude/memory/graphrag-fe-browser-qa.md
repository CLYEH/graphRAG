...
- 定位:**補充驗證步,不取代 Playwright e2e 這個 DoD gate**(CI 跑不了——agent session 限定)。但「有沒有跑過」由 hook 機械強制(owner 同日追加定案,回應「怎麼驗證你不會跳過」):H10 browser-QA receipt——stamp script 要求截圖證據檔存在並記 tree hash,push/PR gate 在 `task/FE*` 分支上無當前 tree 的 receipt 即擋(語意同 review receipt:改 tree 就要重跑重蓋);self-stamp 弱於 reviewer-stamp(H5 教訓),故 PR body 必附截圖/GIF 供 owner 與 Codex 抽查。
