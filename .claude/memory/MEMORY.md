...
- [Loop 流程教訓 + lesson classes](graphrag-loop-paused-pr5.md) — watcher 三管道;Codex 意見先判讀;同類一次掃完;**LOOP step 8 post-merge retro 比對本檔的 11 個 lesson classes**(契約驗值/入口一致性/規則自洽/工具語意/檢查者分岔/spec 對齊/執行級驗證/邊界語意×表示誤差/防護面完整性×過度阻擋/綁定時檢查≠不變量TOCTOU/請求級不變量掃全生命週期);即時狀態查 TASKS.md/GitHub
...
- [BA2 registry FK 待辦](graphrag-ba2-registry-fk.md) — BA2 要讓 build-creation registry-aware + 加 FK builds.project→projects.name(RESTRICT),結構性關掉 BA1a delete_project 的 count-then-delete TOCTOU;backfill 已備妥前置
