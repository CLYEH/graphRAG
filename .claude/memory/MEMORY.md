...
- [CI 先於 Codex](graphrag-ci-before-codex.md) — push 後先 `gh pr checks` 修 CI 紅燈再 triage Codex;poke 前查 eyes+未 triage 回應(兩者都要);等回覆用 watch-codex.sh 別手寫 poll(--paginate 教訓)、triage 中絕不啟 probe;push 後不 poke(Codex 會自動 re-review);切片任務分支名要對到子項(C3a→task/C3a)
