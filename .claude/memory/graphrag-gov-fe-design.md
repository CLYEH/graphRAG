...
description: GOV2-fe+GOV3-fe 治理中心(TASKS.md 125/128,同落地)owner 2026-07-19 定案:分頁式治理頁 + 兩 gap 清單延後另立 api 任務 + 發布閘顯示型;Plan agent 已驗碼,待實作
metadata:
  node_type: memory
  type: project
...
1. GOV3-fe 本體提案池(最小、全服務):`useOntologyProposals`+`useDecideOntologyProposal`
   + ProposalPool 分頁;順手加分頁 host 骨架到 ReviewQueue.tsx。
...
3. Health 深連結:ProjectHealth.tsx `COUNT_LABELS` 各非零訊號 →治理分頁(count>0 才連結)。
   移除 `:77-81` 只 merge 能連結的限制。
4. 發布閘顯示型面板(Overview.tsx ActivateControl 旁)。
