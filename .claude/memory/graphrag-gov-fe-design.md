...
description: GOV2-fe+GOV3-fe 治理中心 owner 2026-07-19 定案:分頁式治理頁 + 兩 gap 清單延後另立 api 任務 + 發布閘顯示型;**GOV3-fe 已 merge(#104,Codex 3 輪);GOV2-fe 拆 fe-1/2/3/4;fe-1 entity(#105)、fe-2 relation(#106)、fe-3 治理待辦面板(#107,Codex 2 輪→scope-honesty:GovernanceBacklog 依真實 scope 分組)皆已 merge;尚餘 fe-4 已決定視圖+影響抽屜+分頁硬化**
metadata:
  node_type: memory
  type: project
...
3. Health 深連結:ProjectHealth.tsx `COUNT_LABELS` 各非零訊號 →治理分頁(count>0 才連結)。
   ...
   (#105)、`needs_review_relations`→relation(#106)皆已落地;低信心/缺證據待 facet api 任務。
...
5. **GOV2-fe-4 審核佇列 robustness**(Codex #105 浮現,TASKS.md 已立):(a)已決定/稽核-復原視圖
   ...
   (c)relation 影響/impact 抽屜(useSubgraph)。
