...
- **兩 gap = 已解(#109 GOV2-facet)**:`/relations` allowlist 現含 `confidence`/`evidence`
  ...
  `LOW_CONFIDENCE_BELOW` 常數(同源單讀,可覆寫參數已移除)。**剩 FE 消費**:兩分頁 +
  Health 深連結——測試必斷言同送 `filter[status]=active`+facet(facet 正交,gauge parity
  靠組合),見 [[graphrag-open-followups]] 的「GOV2 gap-list FE 片」。
...
2. GOV2-fe 實體/關聯審核佇列 → **實際拆 fe-1/2/3/4**:
   ...
   - **GOV2-fe-2 relation 審核 = ✅ 已 merge #106**(Codex **5 輪**全 P1/P2;教訓見
     [[graphrag-loop-paused-pr5]] #106):relation 比 entity 豐富——per-row RelationRow 以 useEntity
...
3. Health 深連結:ProjectHealth.tsx `COUNT_LABELS` 各非零訊號 →治理分頁(count>0 才連結)。
   ...
   (#105)、`needs_review_relations`→relation(#106)皆已落地;低信心/缺證據待 FE gap-list 片(facet api 已 #109 落地)。
4. GOV2-fe-3 = ✅ 已 merge #107(Codex 2 輪皆 scope-honesty,見 [[graphrag-loop-paused-pr5]] #107):
...
5. **GOV2-fe-4 審核佇列 robustness = ✅ 已 merge #108**(Codex 2 輪,見
   [[graphrag-loop-paused-pr5]] #108):(a)已排除/復原視圖(已排除者可 restore;idem-key
