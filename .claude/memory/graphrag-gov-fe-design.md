...
description: GOV2-fe+GOV3-fe 治理中心 owner 2026-07-19 定案:分頁式治理頁 + 兩 gap 清單延後另立 api 任務 + 發布閘顯示型;**GOV3-fe 已 merge(#104,Codex 3 輪);GOV2-fe 拆 fe-1/2/3/4;fe-1 entity(#105,Codex 2 輪→決定性鍵+排除確認)、fe-2 relation(#106,Codex 5 輪→名稱解析+懶載 evidence+每決定入口×資料未就緒態 gate+refetch 窗鎖)皆已 merge;尚餘 fe-3 發布閘/fe-4 已決定視圖+影響抽屜+分頁硬化**
metadata:
  node_type: memory
  type: project
