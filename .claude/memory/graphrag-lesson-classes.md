...
metadata:
  ...
  modified: 2026-07-26T07:00:36.116Z
---
...
規則改了但舊措辭活在其他入口——grep 全部 tracked *.md;凍結集合加成員→grep 既有成員名;雙詞彙→兩者都收斂;新領域錯誤→掃所有 consumer 的翻譯層。行為變更掃**行為 pin**(斷言常數),不只 prose(#89);訊號/計數的 consumer 要**全數枚舉**再改——低信心/缺證據計數有 Health 與 Overview GovernanceBacklog 兩個 consumer,只接一個=另一入口掛著已不實的註記到不了新清單(#119 P2);退役假 affordance/警語→grep 它所有實例(標頭註解、inline、UI 文案,#101);絕不重抄凍結 schema 欄位清單——引用 schema(#53)。UI 可見字串改名必掃 `web/e2e/*.spec.ts`(Playwright 在本地 check-all/push gate 之外;CI 僅 web-touching PR 的非 required `e2e` job 跑它——H18,#104)。改清單排序一次連動五件事:order × keyset × cursor × sort-affordance × index(#99);端點錯誤優先序=資源 404→bootstrap 409→request/config 400,新插入的 config 解析要落在正確層(#120 R5:metadata-schema 400 搶了 NO_ACTIVE_BUILD 409);**degradation 面也是 sibling 面**——修 warning/錯誤訊息時列出同一訊號的所有發射點(單模態 _bounded、hybrid per-mode、introspection error 是三個面),修一漏二會被逐面擠輪(#122 r3 P1:store 具名只修了單模態,預設工具 hybrid 反而漏);共用 helper 使面不可分岔。**移除一個機制=全倉掃它的消費者與宣稱**(#128:刪 LLM selector 後,eval runner 的 models_needed 仍假設 selector 必被諮詢(Codex r1 P2)、docstring/comment 殘留 selector 措辭(gate-2 一輪 5 nit,retro 時 doc-reviewer 又抓到整合 fixture `_SelectorLLM` 一處——「全清」宣稱本身也要被驗證)——刪除前先 grep 機制名+其行為假設,消費者與 prose 一次清)。**對外表面的品質規則(去行話/去內部代號)要枚舉「每一個廣告面」**(#134:工具與參數描述掃完 § 後,把凍結 mcp_response schema 掛成 outputSchema——schema 自帶 ≥17 條 §/DR 描述又整包洩回;修=廣告副本遞迴去 description annotation。掛新工件到對外面=該工件的 prose 也進了同一條規則的轄區;資源路徑解析先 grep 同倉既有雙候選 loader(policy.py)再自寫)。
