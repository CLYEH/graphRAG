...
**Codex 核可的兩種格式(2026-07-07,PR#39 教訓;trace 錨=PR#39 comments)**:push 觸發的自動審 → `+1` **reaction**(hook 認得);`@codex review` poke 觸發的審 → 「Didn't find any major issues」**comment**(指名 Reviewed commit,**無 reaction**,hook 不認)。規則:(1)watcher 必須三態監看——+1 reaction/新 threads/乾淨 comment;(2)**乾淨 review 之後絕不再 poke**(會觸發新一輪邊角 findings);(3)若只有乾淨 comment 無 +1,先等 push 觸發的下一次自動審(通常會留 +1),仍無則回報 owner 裁決——hook 不可擅改(owner 明令)。
