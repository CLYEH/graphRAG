...
**How to apply:** LOOP step 7 只有一個放行條件——Codex 對 head commit +1。若 Codex 持續留言,依 LOOP step 7 的 **suggestion triage** 處理(必要才修;非必要引 DESIGN §/DR 回覆後 resolve),直到它 +1;若它遲遲不 +1,**停下來問使用者**,絕不自行合併。不要再引入任何「gates 滿足即合併」的捷徑。相關:[[graphrag-architecture]]。
...
**已知限制（使用者拍板接受）:** 無法把「乾淨 +1 reaction」真正綁到 head SHA——`Commit.pushedDate`=null、乾淨 approval 無 commit_id;故用 committer.date 當 freshness proxy(對誠實 agent 足夠,能擋 push 新 commit 後的 stale +1;擋不住蓄意 backdate)。另 out-of-scope 邊緣:`gh pr merge --body "..." <ref>`(引號值)、跨 repo `gh api PUT`。**教訓:Codex 審安全控制會無止盡找邊緣 nit,收斂靠使用者決定「API 限制內已足夠即合併」,而非等到永遠沒意見**（PR #3 就是這樣收尾:多輪後由使用者選「接受 committer.date、收尾合併」）。
