...
- [ ] BA2e ingest/build triggers + job endpoints + SSE — design note (class-12 sibling, from #52 retro): the trigger's create_job→enqueue_build gap is an UNMARKED crash window (job committed `queued`, never enqueued → invisible to both arq and the lease reaper, stuck forever); the slice must close or recover it (e.g. reaper also sweeps never-leased `queued` jobs older than a generous grace, or enqueue-in-band with the idempotent retry path re-enqueueing)
