...
- [ ] QA4 REST 靜默 clamp(D4)— `POST /projects/{p}/query/semantic {"top_k":50}` → 200、恰 20 筆、`warnings: []`(top_k=15 → 15 筆,證明是 policy clamp);hybrid 同病。DESIGN §27.2(MCP13)明文「任何 clamp 必發 TRUNCATED,靜默 clamp 禁止」,且 MCP 同請求**有**發、REST `/query/global` 也**有**發——同一表面自相矛盾。機制已定位:MCP13 的 `_with_clamp_warning` 只修在 `core/mcp/server.py`,REST 查詢路由沒跟上。小改動,直接消除兩表面對結果完整性的矛盾。

### 框架層與型別化錯誤(P3,建議一次修掉)
- [ ] QA5 字串/框架層驗證一次收口(D5/D9/D10/D11/D12/D14)— 同一族「畸形輸入逃出型別化錯誤契約」:(a) **NUL byte** 進 Postgres → MCP 回**假的** `STORE_UNAVAILABLE`「postgres unavailable」而 store 其實健康、REST 回 500;路徑段帶 NUL 更誤標 503「registry unreachable — check Postgres」(其他壞名稱都正確 404)——一個 byte 就能讓系統對基礎設施狀態說謊,照契約辦事的 agent 會對健康的 DB 退避重試。(b) **純空白 query** 完全未驗證 → 20 筆有分結果、零警告、rank0 是語料的「（未指明）」佔位 entity;空字串則被送到 OpenAI 才擋,錯誤歸咎 model provider(REST 反而本地驗證)。(c) **pydantic 原始錯誤**繞過承諾的 error_code envelope,外洩內部 class 名 `semantic_searchArguments` 與 `errors.pydantic.dev/2.13/` 版本 URL(全 13 工具皆然)。(d) **lone surrogate** 吐原始 `UnicodeEncodeError`(守規矩的 JS 客戶端 `slice(0,4000)` 切到 emoji 中間即觸發)。(e) **bool→int 強制轉換**打穿 server 自己的 `type(limit) is not int` 防護,`limit:true` 變成 1 筆/頁的「成功」。修法:工具入口統一攔 pydantic/serialization 例外 → typed envelope,並加 NUL/空白/surrogate 的字串前置驗證(REST 側補 q/filter 的 NUL 檢查與 min_length 對齊)。
