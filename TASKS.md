...
- [ ] BA2d-3 **lease reaper** — decouple crash recovery from arq's `job_timeout` (Codex BA2d-2 P1): with a generous job_timeout (so arq never cancels a live build → no stranded SoR row), a periodic arq cron sweeps `building` builds whose heartbeat-lease has expired (crashed/starved worker) and re-enqueues them under a fresh arq `_job_id` so `run_build_leased` re-acquires the now-free lease and resumes — fast crash recovery (~1 min) independent of job_timeout. Completes the BA2d-1 DB-lease design as the SOLE build-liveness authority.
