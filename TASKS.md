...
### P0 — 會導致 agent 給出錯誤答案
- [ ] MCP2 warning 分類正確性(review 第 1/4/13 條)— 三處把「agent 該做什麼」講錯的 typed warning:(a)`graph_query` 的 seed 名稱解析不到時回 `results:[], warnings:[]`,與「確實沒有關聯」完全同形(`core/query/graph.py:492-493`、`:171-172`)——agent 會回答「兩者沒有關聯」;補 typed warning(`GUARDRAIL_BLOCKED` 或新增 `SEED_NOT_FOUND`)並在訊息指向 `get_entity`,hybrid 內的同一個洞(graph_entity 沒解析到卻無警告)一併補。(b)呼叫方輸入錯誤被標成 `STORE_UNAVAILABLE`(`core/query/hybrid.py:220-227` 無差別 `except Exception`;實測 `hybrid_query{query:""}` → `"semantic mode failed (BadRequestError)"`)——agent 會當基礎設施問題無限重試同一個畸形查詢;分流成輸入類→`GUARDRAIL_BLOCKED`、store 類→`STORE_UNAVAILABLE`(`_bounded` 已有此紀律,`tests/test_mcp_server.py:363-368` 有斷言,本條是 hybrid 偏離)。(c)`STORE_UNAVAILABLE` 不說是哪個 store(只給 `ResponseHandlingException` 這種 client 內部術語)——實測 Qdrant 掛掉時 graph 仍回 87 筆、Postgres 掛掉時全滅,agent 該做的事相反卻分不出來;逐模態處理已做對(`graph.py:768`),補 `_bounded` 的泛用信封。
