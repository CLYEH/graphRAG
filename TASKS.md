...
- [ ] BA2e-2 SSE job events — `GET /jobs/{id}/events` (`text/event-stream`; frozen JobEvent: `job.update|job.done|job.failed`, full shape always present — step/message null not absent); poll the jobs SoR (Postgres), NEVER arq (worker `keep_result=0` by design); stream ends after the terminal event
