...
- [ ] H6 migration `0005`: `pipeline_step_items.item_ref <> ''` CHECK (+ `item_kind <> ''`) — the identifier-non-empty rule applied to the P6 table it predates; an empty item_ref is a no-op identity in the §27.7 retry dedup — retro of PR #17 (deferred there as out-of-scope for an already-merged table)
