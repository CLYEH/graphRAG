...
- [x] MCP17 gateway 水平擴充與 session 生命週期:serve-mcp 改 import-string factory+--workers(多進程可用;session process-sticky 需 affinity LB,CLI 明言);session_idle_timeout 預建 manager 注入(1800s——閒置 session 與其 DR-012 policy 快照不再永生);/health 回已掛載專案;已刪專案撤銷由 MCP12 preflight 已修。
