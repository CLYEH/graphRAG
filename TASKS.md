...
- [x] QA2 hybrid_query 零文字頁(D1)— 融合後保 passage 樓地板 `_chunk_floor`(`top_k//2`,沿用 `semantic._fair_page` 形狀;`min(floor,len)` 使稀缺型別不反向擋人)。根因:semantic 自身 §16 排序把較高餘弦的 entity 排在 chunk 前,其 chunk 落在該 mode 的 rank 11+,RRF 只看 mode 內名次,遂被兩個 mode 的 entity 整批擠出頁面。
