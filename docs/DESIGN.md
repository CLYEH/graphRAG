...
每專案 config 定核心 schema。結構化資料規則對映；文件用 PropertyGraph 受 schema 引導抽取，允許 LLM 提議新型別 → 待審池，Console 決定採納（🔧 `ontology.proposal_policy: auto|review`）。
...
- **text_to_cypher**：`readonly`, `allowed_clauses[MATCH,WHERE,RETURN,LIMIT]`, `blocked[CREATE,MERGE,DELETE,SET,REMOVE,CALL]`，**禁 APOC / procedure 呼叫**, `timeout_ms`, `row/hops 上限`。
