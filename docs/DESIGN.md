...
- **MCP10 sidecar 慣例(2026-07-25)**:純目錄來源的 `<name>` 檔可在旁放 `<name>.meta.json`(僅 `context`/`governance` 兩 key;`system`/`schema_version` 為 server-stamped 不可偽造),connector 組成 DR-010 envelope 落 `documents.metadata`——終端可呈現的出處(如真實公開網頁 URL 放 `context.attributes.source_url`)由此進入引用鏈,經 `metadata_exposure` allowlist 投影到 chunk source_refs(UXC1b enrichment);孤兒 sidecar/壞 JSON/偽造 namespace 一律 loud fail。上傳來源走 API 的同 envelope 擷取,不用 sidecar。
