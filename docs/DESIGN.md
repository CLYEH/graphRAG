...
工具：`semantic_search` · `graph_query` · `global_summary` · `sql_query` · `hybrid_query`（預設入口）· `get_entity` · `get_chunk` · `get_document`（MCP5:引用→原文的取回路徑;自省形狀,非 §16）· `list_entities` · `list_chunks` · `list_reports`(MCP9:分頁瀏覽/子字串搜尋+字元-AND 模糊回退——列舉牆的解;游標以 sha256 指紋釘 build+tool+filters+match)· `list_schema` · `explain_retrieval`。所有 retrieval 工具回傳統一 contract（§16），查詢綁定 active build。**MCP14 工具面 metadata**:tools/list 全參數帶 description、封閉詞彙帶 enum,信封工具 outputSchema=凍結 mcp_response 契約(廣告面);server 以 instructions/website_url 自述、serverInfo.version=graphrag 套件版本;未實作的 prompts/resources 能力不宣告。Transport 🔧 stdio/http。**DR-012（CFG1）**:HTTP 供應形狀=`graphrag serve-mcp` 單一 gateway process,每專案一個「邏輯」server 實例掛於 `http://<host>:<port>/mcp/<project_name>`(一 port 多專案;registry 驅動 lazy mount——建完專案免重啟即可連);query_policy/metadata_exposure 一律讀 registry `projects.config`(單一 SoR,廢除 `projects/<name>/config.yaml` 與每專案手寫 entrypoint);gateway 無 auth(§23 placeholder,限內網/tunnel)。
