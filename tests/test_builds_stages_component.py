...
def spy(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    # these hermetic stage tests run on a fake conn (no SQL) — the ingest/graph
    # stages' RB1-retry check hits the DB, so stub it to the non-retry path they
    # exercise (None = ordinary build, not a retry child)
    async def _not_retry(conn: Any, build_id: uuid.UUID) -> uuid.UUID | None:
        return None

    monkeypatch.setattr(stages_mod, "_retry_parent", _not_retry)
