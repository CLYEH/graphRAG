...
def test_browser_stamp_refuses_a_claim_without_evidence(toy_repo: Path) -> None:
    ...
    # ...and evidence that would ENTER the content tree refuses too: an
    # unignored in-repo artifact perturbs the snapshot the receipt binds,
    # deadlocking the worktree==HEAD gate (the push could never succeed)
    tracked = toy_repo / "shot.png"
    tracked.write_bytes(b"\x89PNG fake-but-non-empty")
    unignored = _run([BASH, BROWSER_STAMP_SCRIPT, tracked.as_posix()], toy_repo)
    assert unignored.returncode != 0, "unignored in-repo evidence stamped a receipt"
    assert "gitignored" in unignored.stderr
    receipts = toy_repo / ".claude" / "receipts"
...
def test_off_checkout_fe_ref_is_checked_before_the_no_op_shortcut(toy_repo: Path) -> None:
    """The ORDERING is the security property: on a CLEAN checkout with nothing
    outgoing, the gate's no-op fast path exits 0 — so an off-checkout FE ref
    push would ride straight out, skipping both the ref check and the browser
    receipt. The walk must run BEFORE that shortcut. Discriminating: moving it
    after the fast path returns 0 here."""
    assert BASH is not None
    (toy_repo / ".gitignore").write_text(
        ".claude/receipts/\norigin.git/\nshim/\n", encoding="utf-8"
    )
    _origin_and_branch(toy_repo, "task/FE1")
    (toy_repo / "a.md").write_text("unreviewed fe content\n", encoding="utf-8")
    subprocess.run(["git", "commit", "-qam", "fe"], cwd=toy_repo, check=True, capture_output=True)
    # back to main: worktree clean, NOTHING outgoing vs origin/main — the
    # no-op shortcut's exact precondition, while task/FE1 stays divergent
    subprocess.run(["git", "switch", "-q", "main"], cwd=toy_repo, check=True, capture_output=True)
    env = _uv_shim(toy_repo)

    denied = _run([BASH, GATE_SCRIPT], toy_repo, stdin=f"{_PUSH} origin task/FE1", extra_env=env)
    assert denied.returncode == 2, f"FE ref rode the no-op shortcut: {denied.stdout}"
    assert "HEAD:<dst>" in denied.stderr
