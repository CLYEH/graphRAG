...
from core.stores.tables import builds, community_reports
...
async def _report(writer: BuildScopedWriter, title: str, rating: float | None) -> None:
    await writer.insert(
        community_reports,
        id=uuid.uuid4(),
        level=0,
        ...
        member_entity_ids=[uuid.uuid4(), uuid.uuid4()],
        rating=rating,
    )
